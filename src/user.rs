...
impl User {
    pub async fn login(email: &str, password: &str, db: &DatabaseConnection) -> Result<Self, Error> {
        let user = UserEntity::find().filter(<UserEntity as EntityTrait>::Column::Email.eq(email)).one(db).await?;
        if user.is_none() {
        ...
        let hashed_pass = Self::hash_password(password, real_hash.salt.expect("password did not have a salt"))?;
    ...
    pub async fn register(email: &str, password: &str, db: &DatabaseConnection) -> Result<Self, Error> {
        todo!("validate email");
   ...
   fn hash_password<'t>(cleartext: &str, salt: argon2::password_hash::Salt<'t>) -> Result<argon2::password_hash::PasswordHash<'t>, Error> {
        let a = Self::argon2();
